...
async function fileToPdfEmbedHtml(fotoPath: string, lang: AppLanguage): Promise<string> {
  const S = pdfStrings(lang);
  try {
    const info = await FileSystem.getInfoAsync(fotoPath);
    if (!info.exists) return `<p class="attNote">${safeText(S.attachmentMissing)}</p>`;
  } catch {
  ...
  if (ext === 'heic' || ext === 'heif') {
    try {
      const converted = await manipulateAsync(fotoPath, [], {
        compress: 0.85,
      ...
      if (converted.base64) {
        try {
          await FileSystem.deleteAsync(converted.uri, { idempotent: true });
        } catch {
          /* temp file cleanup best-effort */
        }
        return `<div class="attImgWrap"><img class="attImg" src="data:image/jpeg;base64,${converted.base64}" alt="" /></div>`;
      }
    } catch {
      /* fall through to unsupported note */
    }
  ...
  if (imageExts.includes(ext)) {
    try {
      const b64 = await FileSystem.readAsStringAsync(fotoPath, { encoding: 'base64' });
      const mime =
  ...
  if (ext === 'pdf') {
    const fname = fotoPath.split('/').pop() ?? 'file.pdf';
...
async function printHtmlToPdfBuffer(html: string, landscape: boolean): Promise<Uint8Array> {
  const res = await Print.printToFileAsync({
    ...
    orientation: landscape ? Print.Orientation.landscape : Print.Orientation.portrait,
  } as Parameters<typeof Print.printToFileAsync>[0]);
  return readFileUriAsUint8Array(res.uri);
}
...
export async function generatePdfForMonth(
  ...
  await FileSystem.writeAsStringAsync(dest, uint8ArrayToBase64(outBytes), { encoding: 'base64' });
  return { uri: dest, filename };
}

export async function shareFile(uri: string, _suggestedName?: string): Promise<void> {
