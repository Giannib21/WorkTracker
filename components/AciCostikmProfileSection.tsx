...
const COST_FETCH_COUNTDOWN_START = 75;
...
type Props = {
  ...
  /** Se true, niente `Card` esterna: contenuto da inserire nella stessa scheda «Auto / Rimborso km». */
  embedded?: boolean;
  /** Es. per mostrare il link al calcolatore ufficiale quando in automatico c’è un errore. */
...
export function AciCostikmProfileSection({
  ...
  if (embedded) {
    return <View style={styles.embeddedRoot}>{aciBody}</View>;
  }

  return (
    <Card mode="outlined">
      <Card.Content style={{ gap: 10 }}>{aciBody}</Card.Content>
    </Card>
  );
}
